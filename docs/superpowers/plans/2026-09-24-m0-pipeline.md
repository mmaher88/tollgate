...
- **O4** Run `tooling/asc/provision.py setup` (Task 4). After it creates the two App IDs, open each in the portal, enable App Groups > Configure, tick the group, save; then run `tooling/asc/provision.py profiles`.
...
Run: `uvx actionlint .github/workflows/*.yml` (if `actionlint` is not on PyPI, use `docker run --rm -v "$PWD:/repo" -w /repo rhysd/actionlint:latest`)
Expected: no errors.

Run: `bash -n tooling/ci/*.sh tooling/scripts/*.sh && shellcheck tooling/ci/*.sh tooling/scripts/*.sh` (shellcheck through `docker run --rm -v "$PWD:/mnt" koalaman/shellcheck:stable` if not installed)
Expected: no errors.

- [ ] **Step 5: Commit**

```bash
git add tooling/ci .github/workflows/ios.yml
...
**Interfaces:**
- Consumes: `tooling/config.env`; environment `ASC_KEY_ID`, `ASC_ISSUER_ID` (empty for an individual key), `ASC_KEY_PATH`.
